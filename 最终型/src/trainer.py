...
class Trainer():
    ...
    def train(self):
        self.loss.step()  # 执行一次优化器的优化步骤。
        epoch = self.optimizer.get_last_epoch()  # 获取当前训练的轮数。
        lr = self.optimizer.get_lr()  # 获取当前的学习率。

        self.ckp.write_log(
            '[Epoch {}]\tLearning rate: {:.2e}'.format(epoch, Decimal(lr))
        )  # 将当前的轮数和学习率写入日志文件。
        self.loss.start_log()  # 开始记录损失值。
        self.model.train()  # 将模型设置为训练模式。

        timer_data, timer_model = utility.timer(), utility.timer()  # 创建计时器对象。
        # TEMP
        self.loader_train.dataset.set_scale(0)  # 设置训练数据集的尺度为0。
        for batch, (lr, hr, _,) in enumerate(self.loader_train):  # 遍历训练数据集的每个批次。
            lr, hr = self.prepare(lr, hr)  # 对输入数据进行预处理。
            timer_data.hold()
            timer_model.tic()

            self.optimizer.zero_grad()  # 将优化器的梯度缓冲区清零。
            sr = self.model(lr, 0)  # 使用模型对低分辨率图像进行超分辨率重建，得到高分辨率图像。
            loss = self.loss(sr, hr)  # 计算重建图像与真实高分辨率图像之间的损失。
            loss.backward()  # 根据损失值计算梯度。
            if self.args.gclip > 0:  # 如果设置了梯度裁剪的阈值，则对模型的梯度进行裁剪。
